...
const Nav: React.FC = () => {
    ...
    return (
        <>
            <nav className={`top-0 left-0 w-full z-[100] bg-white transition-all duration-300 ${
                scrolled ? 'bg-white/80 backdrop-blur-md py-4 shadow-sm' : 'bg-transparent py-6 md:py-8'
            }`}>
                <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
                    
                    {/* Logo */}
                    <a href="/" className="z-[60] text-white text-2xl p-2 rounded-full          " 
  style={{ backgroundColor: brandColor }} >
                    ...
                    <button 
                        ...
                        aria-label="Menu"
                    >
                        <span className={`h-0.5 w-6 transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-2 bg-white' : 'bg-white'}`}></span>
                    ...
                    `}>
                        <ul className="flex flex-col md:flex-row items-center gap-8 md:gap-10 list-none">
                            {['Home', 'About Me', 'My portfolio'].map((item) => {
